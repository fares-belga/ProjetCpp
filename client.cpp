...
bool Client::exporterPDF(QString fileName, QSqlQueryModel* model)
{
    QFile file(fileName);
...
Client Client::clientFidele()
{
    QSqlQuery query;
          ...
          if (query.next()) {
              ...
              QString mail = query.value(1).toString(); // Remplacez ces valeurs par les colonnes appropriées
              QString adr = query.value(2).toString();
              int nbra = query.value(3).toInt();

              // Créez un objet Employer avec les données récupérées
              Client cl(cin, mail, adr,nbra); // Assurez-vous d'inclure toutes les informations nécessaires
